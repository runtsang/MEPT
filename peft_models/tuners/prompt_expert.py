...
@dataclass
class PromptExpertConfig(PromptLearningConfig):
    """
    ...
    shared_expert_ratio: Optional[str]= field(
        default=None
    )
    shared_expert: Optional[bool]= field(
        default=None
    )
    prompt_initialization: Optional[str]= field(
        default=None
    )
    # probss = torch.zeros((1, 1), dtype=torch.float)
    def __post_init__(self):
        self.peft_type = PeftType.PROMPT_EXPERT


class PromptExpertEncoder(torch.nn.Module):
    ...
    def forward(self, indices, input_ids, inputs_embeds, attention_mask, base_model=None):
        ...
        return prompt_embeddings, probs
