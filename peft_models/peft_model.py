...
from .tuners import LoraModel, PrefixEncoder, PromptEmbedding, PromptEncoder, PromptRoutingEmbedding, PromptExpertEncoder
from .utils import (
    ...
    **Attributes**:
    ...
    def _setup_prompt_encoder(self):
        ...
        if self.peft_config.peft_type == PeftType.PROMPT_TUNING:
            ...
            print("routing")
        elif self.peft_config.peft_type == PeftType.PROMPT_EXPERT:
            prompt_encoder = PromptExpertEncoder(self.peft_config, self.word_embeddings)
            print("expert")
        else:
        ...
        **kwargs,
        ...
        if self.peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            if self.peft_config.peft_type == PeftType.PROMPT_ROUTING:
                ...
                prompts, probs = self.get_prompt_routing(batch_size=batch_size, input_ids=input_ids, inputs_embeds=inputs_embeds, attention_mask=attention_mask)
                # print(probs)
            else:
                prompts = self.get_prompt(batch_size=batch_size)
            ...
            if self.peft_config.peft_type == PeftType.PROMPT_EXPERT:
                return self.base_model(inputs_embeds=inputs_embeds, decoder_inputs_embeds=decoder_inputs_embeds, probs=probs, **kwargs)
            else:
        ...
        if not isinstance(self.peft_config, PromptLearningConfig):
            ...
            if self.peft_config.peft_type == PeftType.PREFIX_TUNING:
                ...
                if self.peft_config.peft_type == PeftType.PROMPT_ROUTING:
                    ...
                    prompts, probs = self.get_prompt_routing(batch_size=batch_size, input_ids=input_ids, inputs_embeds=inputs_embeds, attention_mask=kwargs["attention_mask"])
                else:
                    prompts = self.get_prompt(batch_size=batch_size)
                if prompts.shape[1] == batch_size:
                    prompts = prompts.transpose(0, 1)
