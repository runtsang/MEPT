...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, block_idx=None):
        super().__init__()
        self.is_decoder = config.is_decoder
        ...
        self.config = config
        if config.method == "prompt-expert" and not self.is_decoder and self.block_idx in config.layers:
            ...
            if self.config.shared_expert:
                if config.shared_expert_ratio == 'one':
                    self.num_shared_experts = config.num_virtual_tokens
                elif config.shared_expert_ratio == 'half':
                    assert config.num_virtual_tokens_full % 2 == 0
                    self.num_shared_experts = config.num_virtual_tokens_full // 2

                self.shared_embedding = torch.nn.Parameter(torch.zeros(1, self.num_shared_experts, config.token_dim))
            
            if config.prompt_initialization == "xavier":
                torch.nn.init.xavier_uniform_(self.prompt_embedding.weight)
                # torch.nn.init.xavier_uniform_(linear.weight.data)
                if self.config.shared_expert:
                    torch.nn.init.xavier_uniform_(self.shared_embedding)
            elif config.prompt_initialization == "he":
                torch.nn.init.kaiming_uniform_(self.prompt_embedding.weight)
                # torch.nn.init.kaiming_uniform_(linear.weight.data)
                if self.config.shared_expert:
                    torch.nn.init.kaiming_uniform_(self.shared_embedding)
            elif config.prompt_initialization == "orthogonal":
                torch.nn.init.orthogonal_(self.prompt_embedding.weight)
                if self.config.shared_expert:
                    torch.nn.init.orthogonal_(self.shared_embedding)
    ...
    def print_and_reset_load_counts(self):
        print(self.load_counts.long().tolist())
        probs = [round(v, 2) for v in self.probs_avg.tolist()]
        print(probs)
        self.reset_load_counts()
    ...
    def forward(
        ...
        if self.config.method == "prompt-expert" and not self.is_decoder and self.block_idx in self.config.layers:
            # print(hidden_states.size(), attention_mask.size())
            batch_size, ln, dimm = hidden_states.shape[0], hidden_states.shape[1],hidden_states.shape[2]
            num_virtual_tokens_full = self.config.num_virtual_tokens_full
            num_virtual_tokens = self.config.num_virtual_tokens
            # am = attention_mask.squeeze().squeeze()
            # print(am[0])
            hiddens = self.layer_norm(hidden_states)
            hiddens = hiddens[:, self.config.num_virtual_tokens:, :]
            # attention_mask = torch.ones(batch_size, ln).cuda()
            # # attention_mask = torch.cat((prefix_attention_mask, attention_mask), dim=1)
            # sentence_sum = torch.sum(hiddens * attention_mask.squeeze().squeeze(), dim=1)
            # non_zero_count = torch.clamp(torch.sum(attention_mask.squeeze().squeeze(), dim=1, keepdim=True), min=1)
            # print(non_zero_count[0], non_zero_count[-1])
            ...
            # l1_norm = torch.norm(sentence_sum, p=1, dim=1, keepdim=True)
            # sentence_sum /= l1_norm
            # non_zero_count = torch.clamp(torch.sum(am, dim=1, keepdim=True), min=1)
            non_zero_count = torch.full((batch_size, 1), ln, dtype=torch.float, device='cuda')
            scaling_factor = torch.sqrt(torch.tensor(dimm, dtype=torch.float32))
            # sentence_embeds = sentence_sum / (non_zero_count.float() * scaling_factor)
            sentence_embeds = sentence_sum / (non_zero_count.float())

            if not self.config.stochastic:
                probs = self.prompt_router(sentence_embeds)
                # probs = self.config.probss.cuda()
                if self.config.gumbel:
                    probs = gumbel_softmax(probs, k=1, hard=True)
                probs_mean = torch.mean(probs, dim=0)
                self.probs_avg = probs_mean
            ...
            # print(self.block_idx, self.load_counts)
            if k == 1:
            ...
            if self.config.shared_expert:
                prompt_embeddings = prompt_embeddings + self.shared_embedding
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        self.block = nn.ModuleList(
            [T5Block(config, has_relative_attention_bias=bool(i == 0), block_idx=i) for i in range(config.num_layers)]
        )
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        print("initializing T5ForConditionalGeneration")
