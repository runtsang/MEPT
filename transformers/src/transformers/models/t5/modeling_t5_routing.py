...
class Router(nn.Module):
    def __init__(self, hidden_dim, topk):
    ...
    def forward(self, hidden, prompt):
        # hidden: [batch_size, num_heads, seq_len_hidden, hidden_dim]
        # prompt: [batch_size, num_heads, num_prompts, hidden_dim]

        # Project hidden and prompt to query and key/value
        query = self.query_layer(hidden)  # [batch_size, num_heads, seq_len_hidden, hidden_dim]
        key = self.key_layer(prompt)      # [batch_size, num_heads, num_prompts, hidden_dim]
        # value = self.value_layer(prompt)  # [batch_size, num_heads, num_prompts, hidden_dim]

        # Calculate attention scores
        attention_scores = torch.matmul(query, key.transpose(-2, -1))  # [batch_size, num_heads, seq_len_hidden, num_prompts]
        attention_scores = attention_scores / (self.hidden_dim ** 0.5)  # Scale by sqrt(hidden_dim)

        # Apply softmax to get attention weights
        attention_weights = F.softmax(attention_scores, dim=-1)  # [batch_size, num_heads, seq_len_hidden, num_prompts]

        # Sum attention weights over the sequence length of hidden
        attention_weights_sum = torch.sum(attention_weights, dim=-2)  # [batch_size, num_heads, num_prompts]

        # Get the indices of the top 5 prompts based on the attention weights
        topk_values, topk_indices = torch.topk(attention_weights_sum, k=self.topk, dim=-1)  # [batch_size, num_heads, 5]

        # Gather the top 5 prompt embeddings
        batch_size, num_heads, num_prompts, hidden_dim = key.size()
        topk_indices = topk_indices.unsqueeze(-1).expand(-1, -1, -1, hidden_dim)  # [batch_size, num_heads, 5, hidden_dim]
        topk_prompts = torch.gather(prompt, dim=-2, index=topk_indices)  # [batch_size, num_heads, 5, hidden_dim]
        
        topk_values = topk_values.unsqueeze(-1).expand(-1, -1, -1, hidden_dim)
        topk_prompts *= topk_values
...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        # self.prompt_router = Router(config.d_model//config.num_heads, config.num_virtual_tokens)
        ###################
        # self.config = config
        ...
        #     1, config.num_virtual_tokens_full, config.token_dim)).cuda()
        # # xavier_uniform initialization
        # # print(self.prompt_embeddings)
        # # import torch.nn.init as init
        # # with torch.no_grad():
        # #     init.xavier_uniform_(self.prompt_embeddings)
        # # print(self.prompt_embeddings)
        # # nn.init.uniform_(self.prompt_embeddings.data, -1, 1)
        # # print("i am here")
        # self.prompt_dropout = torch.nn.Dropout(p=0.2)
    ...
    def expert_forward(self, inputs_embeds):
        batch_size = inputs_embeds.shape[0]

        hiddens = inputs_embeds
        sentence_embeds = torch.sum(hiddens, dim=1)
    ...
    def forward(
        ...
        def project(hidden_states, proj_layer, key_value_states, past_key_value):
            ...
            if past_key_value is not None:
                if key_value_states is None:
                    ...
                    # print(hidden_states.size(), past_key_value.size())
                    # print(layer_head_mask.size())
                    # print(mask.size())
                    # print("1")
                    # print(past_key_value.size())
                    # print(past_key_value.requires_grad)
                    
                    # batch_size, n_heads, key_length, dim_per_head = past_key_value.shape[0], past_key_value.shape[1], past_key_value.shape[2], past_key_value.shape[3]
                    
                    # # permuted_tensor = past_key_value.permute(0, 2, 1, 3)  
                    # # past_key_value = permuted_tensor.reshape(permuted_tensor.shape[0], permuted_tensor.shape[1], -1)
                    # permuted_hidden = hidden_states.permute(0, 2, 1, 3)  
                    # permuted_hidden = permuted_hidden.reshape(permuted_hidden.shape[0], permuted_hidden.shape[1], -1)
                    
                    # past_key_value = self.expert_forward(permuted_hidden)
                    
                    # past_key_value = past_key_value.reshape(batch_size, n_heads, self.config.num_virtual_tokens, dim_per_head)
                    
                    # batch_size, n_heads, key_length, dim_per_head = past_key_value.shape[0], past_key_value.shape[1], past_key_value.shape[2], past_key_value.shape[3]
                    # prompt_embeddings = self.prompt_dropout(self.prompt_proj(self.prompt_embeddings).expand(batch_size, -1, -1))
                    # prompt_embeddings = prompt_embeddings.reshape(batch_size, n_heads, self.config.num_virtual_tokens_full, dim_per_head)
                    
                    # print(hidden_states.size(), past_key_value.size())
                    # pass
                    # hidden_states = torch.cat([self.prompt_router(hidden_states, past_key_value), hidden_states], dim=2)
        ...
        # print(position_bias_masked.size(), scores.size())
        if position_bias_masked.size() != scores.size():
            # 对 position_bias 进行适当的切片操作，确保与 scores 形状匹配
            position_bias_masked = position_bias_masked[:, :, :, :scores.size(-1)]
            # print(position_bias_masked.size(), scores.size())
        # if position_bias_masked.size() == scores.size():
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        self.config = config
        if config.method == "prompt-routing":
            total_virtual_tokens = config.num_virtual_tokens_full # * config.num_transformer_submodules
            self.prompt_embedding = torch.nn.Embedding(total_virtual_tokens, config.token_dim)
            ...
            if self.config.perturb_router:
                sigma = 1
                self.prompt_router = torch.nn.Sequential(
                    linear,
                    GaussianNoise(sigma=sigma),
                    # torch.nn.Dropout(p=0.2)
                )        
            ...
            self.prompt_router.add_module("softmax", torch.nn.Softmax(dim=-1))
            self.load_counts = torch.zeros((self.n_routes,), device='cuda', requires_grad=False)
            self.probs_sum = torch.zeros_like(self.load_counts)
            self.analysis = False
    ...
    def forward(
        ...
        if self.config.method == "prompt-routing" and attention_mask.shape[1] == 1 and attention_mask.shape[2] == 1:
        # if self.config.method == "prompt-routing":
            # print(hidden_states.size(), attention_mask.size())
            batch_size = hidden_states.shape[0]
            num_virtual_tokens_full = self.config.num_virtual_tokens_full
            num_virtual_tokens = self.config.num_virtual_tokens

            hiddens = hidden_states
            am = attention_mask.squeeze(-2).squeeze(-2)
            sentence_sum = torch.sum(hiddens, dim=1)
            non_zero_count = torch.clamp(torch.sum(am, dim=1, keepdim=True), min=1)
            sentence_embeds = sentence_sum

            if not self.config.stochastic:
                probs = self.prompt_router.forward(sentence_embeds)
                if self.config.gumbel:
                    probs = gumbel_softmax(probs, k=1, hard=True)
                probs_mean = torch.mean(probs, dim=0)
                load_routes = torch.argmax(probs.detach(), dim=1)
            ...
            if not self.config.stochastic:
                prompt_embeddings = self.prompt_embedding(idx) * values.unsqueeze(-1).unsqueeze(-1)
                prompt_embeddings = torch.sum(prompt_embeddings, dim=1).squeeze()
                balancing_factor = probs_mean * load_counts #  probs_mean * load_counts 
            else:
                if self.training:
                    prompt_embeddings = self.prompt_embedding(idx)
                else:
                    prompt_embeddings = torch.mean(torch.stack(torch.chunk(self.prompt_embedding.weight.data, self.n_routes, dim=0)), dim=0)
                    prompt_embeddings = prompt_embeddings.repeat(batch_size, 1, 1)
            if prompt_embeddings.dim() == 2:
                prompt_embeddings = prompt_embeddings.unsqueeze(0)
            ...
            hidden_states = torch.cat((
                        prompt_embeddings,
                        hidden_states[:, self.config.num_virtual_tokens:, :]
                    ), dim=1)
